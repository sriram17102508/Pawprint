...
function LostFoundPage() {
  const [tab, setTab] = useState("report"); // report | found | browse
  ...
  // Sample listings
  const listings = [
    { type: "lost", name: "Max", species: "Dog", breed: "Golden Retriever", color: "Golden", area: "RS Puram, Coimbatore", date: "May 18, 2026", img: P.dog4, contact: "98765 43210", reward: "Yes — ₹5,000" },
    { type: "found", name: "Unknown", species: "Cat", breed: "Unknown", color: "Grey tabby", area: "Gandhipuram, Coimbatore", date: "May 20, 2026", img: P.cat2, contact: "87654 32109", reward: "N/A" },
    { type: "lost", name: "Cleo", species: "Dog", breed: "Beagle", color: "Tri-colour", area: "Saibaba Colony", date: "May 19, 2026", img: P.dog2, contact: "76543 21098", reward: "Yes — ₹2,000" },
    { type: "found", name: "Unknown", species: "Dog", breed: "Labrador mix", color: "Black & white", area: "Peelamedu", date: "May 21, 2026", img: P.dog3, contact: "65432 10987", reward: "N/A" },
  ];
      ...
      <div style={{ background: C.white, borderBottom: `1px solid ${C.border}`, position: "sticky", top: 96, zIndex: 50 }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 32px", display: "flex", gap: 0 }}>
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: "16px 28px", border: "none", background: "none", cursor: "pointer", fontSize: 15, fontWeight: tab === t.id ? 700 : 500, color: tab === t.id ? C.orange : C.inkSft, borderBottom: tab === t.id ? `3px solid ${C.orange}` : "3px solid transparent", transition: "all .2s" }}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <section style={{ padding: "48px 0 100px", background: C.cream }}>
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 32px" }}>
          ...
          {tab === "report" && (
            <div>
              {/* Lost / Found toggle */}
              <div style={{ display: "flex", gap: 12, marginBottom: 36, padding: "6px", background: C.white, borderRadius: 14, border: `1px solid ${C.border}`, width: "fit-content" }}>
                {[
              ...
              {type === "lost" && (
                <div>
                  <div style={{ background: C.redLt, borderRadius: 16, padding: "16px 20px", border: `1px solid ${C.red}33`, marginBottom: 28 }}>
                    <p style={{ fontSize: 14, color: C.red, fontWeight: 600 }}>🚨 Act fast — most pets are found within the first 24 hours. Fill in as much detail as possible.</p>
                  </div>
              ...
              {type === "found" && (
                <div>
                  <div style={{ background: C.greenLt, borderRadius: 16, padding: "16px 20px", border: `1px solid ${C.green}33`, marginBottom: 28 }}>
                    <p style={{ fontSize: 14, color: C.green, fontWeight: 600 }}>🙏 Thank you for helping! Fill in as much as you can — even partial info helps match with lost reports.</p>
                  </div>
          ...
          {tab === "browse" && (
            <div>
              <div style={{ display: "flex", gap: 10, marginBottom: 28, padding: "16px 20px", background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, flexWrap: "wrap", alignItems: "center" }}>
                <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: ".06em", textTransform: "uppercase", color: C.inkSft }}>Filter:</span>
                {[["All", "All"], ["Lost 🔴", "lost"], ["Found 🟢", "found"]].map(([l, v]) => (
                  <button key={v} style={{ padding: "7px 16px", borderRadius: 100, border: `1.5px solid ${C.border}`, background: "transparent", color: C.inkSft, fontSize: 13, cursor: "pointer", transition: "all .18s" }}
                    onMouseEnter={e => e.currentTarget.style.borderColor = C.orange} onMouseLeave={e => e.currentTarget.style.borderColor = C.border}>{l}</button>
                ))}
                <input placeholder="Search by breed, area, city…" style={{ ...inputStyle, maxWidth: 240, marginLeft: "auto" }} onFocus={e => e.target.style.borderColor = C.orange} onBlur={e => e.target.style.borderColor = C.border} />
              </div>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
                {listings.map((l, i) => (
                  <div key={i} className="card card-lift" style={{ borderTop: `4px solid ${l.type === "lost" ? C.red : C.green}` }}>
                    <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", alignItems: "stretch" }}>
                      ...
                      <div style={{ padding: "20px 22px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                          <div>
                            <div className={`pill ${l.type === "lost" ? "pill-red" : "pill-green"}`} style={{ marginBottom: 6 }}>{l.type === "lost" ? "🔴 LOST" : "🟢 FOUND"}</div>
                            <div className="melody" style={{ fontSize: 18, fontWeight: 700, color: C.ink }}>{l.name}</div>
                          </div>
                        </div>
                        <div style={{ fontSize: 13, color: C.inkSft, lineHeight: 1.6 }}>
                          <div>{l.species} · {l.breed} · {l.color}</div>
                          <div style={{ marginTop: 4 }}>📍 {l.area}</div>
                          <div style={{ marginTop: 2 }}>📅 {l.date}</div>
                          {l.reward !== "N/A" && <div style={{ marginTop: 4, color: C.orange, fontWeight: 600 }}>🏆 Reward: {l.reward}</div>}
                        </div>
                        <button className="btn btn-sm btn-outline" style={{ marginTop: 12 }} onClick={() => alert(`Contact: ${l.contact}`)}>📞 Contact</button>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div style={{ textAlign: "center", marginTop: 36 }}>
                <button className="btn btn-lg btn-primary" onClick={() => setTab("report")}>+ Report a Pet</button>
              </div>
            </div>
          )}
        </div>
